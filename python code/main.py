...
def Reporter(tuner, max_report_frequency = 5, hyper_num = 1):
    ...
    while True:
        hypers, accuracy, state, perturbs, running_trial_num = ray.get(tuner.get_for_reporter.remote())
...
class Tuner(object):
    ...
    def create_new_trial(self):        
        ...
        if len(self.trials_scheduler):
            while True:
                ...
                if self.resource[resource_id]["calculate_ability"] == 0 or self.resource[resource_id]["calculate_ability"] > remaining_generations:    # 讓很差的資源訓練進度最快的        
                    ...
                    if (self.stop_iteration - self.trials_state[id]["iteration"]) < self.checkpoint_interval / 2:
                        self.checkpoints[id]["checkpoint_interval"] = self.stop_iteration - self.trials_state[id]["iteration"]
                    else:
                        self.checkpoints[id]["checkpoint_interval"] = int(self.checkpoint_interval / 2)
                    cc = 0.5
                else:
                    id = self.trials_scheduler.pop(0)
                    if (self.stop_iteration - self.trials_state[id]["iteration"]) < self.checkpoint_interval:
                        self.checkpoints[id]["checkpoint_interval"] = self.stop_iteration - self.trials_state[id]["iteration"]
                    else:
                        self.checkpoints[id]["checkpoint_interval"] = self.checkpoint_interval
    ...
    def report_before_trial_end(self, id, accuracy, run_time, checkpoint):  
        ...
        if checkpoint["checkpoint_interval"] >= self.checkpoint_interval: # 檢查是否達到突變時間間隔
            mutation.remote(self.tuner, id, self.hypers, self.trial_acc_list, self.last_checkpoint, self.hyperparam_mutations)
        ...
        save_acc_to_json.remote(id, accuracy, self.trials_state[id]["iteration"], self.path)

        if self.resource[resource_id]["Used_count"] == 0.5:
            self.min_run_one_interval_time = min(self.min_run_one_interval_time, run_time)
    ...
    def insert_trial(self, trial_id):
        ...
        while left < right:
            mid = (left + right) // 2
            if self.trials_state[self.trials_scheduler[mid]]["iteration"] <= new_iteration:
                left = mid + 1
            else:
                right = mid
    ...
    def get_for_reporter(self):
        return self.hypers, self.trial_acc_list, self.trials_state, self.perturbs, self.running_trial_num
...
def Trial(tuner, id, hyper, checkpoint): 
    ...
    train_loader, test_loader = get_data_loader(model_type, hyper.get("batch_size", 512))   # 建立訓練資料的loader
    ...
    optimizer = optim.SGD(model.parameters(), lr=hyper.get("lr", 0.01), momentum=hyper.get("momentum", 0.9))
    ...
    if torch.cuda.is_available():                               # 如果是GPU，把model與optimizer放入cpu，因為Checkpoint在CPU
        # Convert to CPU
        for k, v in checkpoint["model_state_dict"].items():
    ...
    tuner.report_before_trial_end.remote(id, acc, run_time, checkpoint)                   # 把結果傳回Tuner
